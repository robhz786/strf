...
constexpr STRF_HD bool is_surrogate(unsigned long codepoint)
{
    return codepoint >> 11 == 0x1B;
}
constexpr STRF_HD bool is_high_surrogate(unsigned long codepoint) noexcept
{
    return codepoint >> 10 == 0x36;
}
constexpr STRF_HD bool is_low_surrogate(unsigned long codepoint) noexcept
{
    return codepoint >> 10 == 0x37;
}
constexpr STRF_HD bool not_surrogate(unsigned long codepoint)
{
    return codepoint >> 11 != 0x1B;
}
constexpr STRF_HD  bool not_high_surrogate(unsigned long codepoint)
{
    return codepoint >> 10 != 0x36;
}
constexpr STRF_HD  bool not_low_surrogate(unsigned long codepoint)
{
    return codepoint >> 10 != 0x37;
}
...
inline STRF_HD bool first_2_of_3_are_valid(unsigned x, bool allow_surr)
{
    return /*0x1F < x && */(allow_surr || (x >> 5) != 0x1B);
}
inline STRF_HD bool first_2_of_3_are_valid(std::uint8_t ch0,  std::uint8_t ch1, bool allow_surr)
{
...
STRF_STATIC_LINKAGE
strf::cv_result STRF_HD cesu8_to_utf32_transcode
