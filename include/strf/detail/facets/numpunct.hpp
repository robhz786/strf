...
constexpr STRF_HD unsigned sep_count(strf::digits_grouping it, unsigned digcount) noexcept
{
    unsigned count = 0;
    while(1) {
...
constexpr STRF_HD strf::digits_distribution calculate_distribution
    ( strf::digits_grouping grouping, unsigned digcount) noexcept;
...
public:
    ...
    constexpr static unsigned grp_max = strf::digits_grouping::grp_max;
    constexpr static unsigned grps_count_max = strf::digits_grouping::grps_count_max;
    ...
    ( strf::digits_grouping grouping, unsigned digcount) noexcept
{
    ...
    while(1) {
        auto grp = grouping.lowest_group();
        STRF_ASSERT(grp);
        if (grp >= digcount || grouping.no_more_sep()) {
...
public:
    ...
    constexpr STRF_HD numpunct(int grp0, int grp1, Grps... grps) noexcept
        : grouping_(grp0, grp1, grps...)
    {
    }
    constexpr STRF_HD numpunct(strf::digits_grouping grps) noexcept
        : grouping_(grps)
    {
    }
    ...
    constexpr STRF_HD strf::digits_distribution grouping(unsigned digcount) const noexcept
    {
        return strf::calculate_distribution(this->grouping(), digcount);
    }
    constexpr STRF_HD bool no_group_separation(unsigned digcount) const noexcept
    {
        return grouping_.no_more_sep() || digcount <= grouping_.lowest_group();
    }
    constexpr STRF_HD unsigned thousands_sep_count(unsigned digcount) const noexcept
    {
        return strf::sep_count(grouping_, digcount);
    }
