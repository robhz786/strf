...
// -DSTRF_RYU_DEBUG Generate verbose debugging output to stdout.
//
// -DSTRF_RYU_ONLY_64_BIT_OPS Avoid using uint128_t or 64-bit intrinsics. Slower,
//     depending on your compiler.
//
// -DSTRF_RYU_OPTIMIZE_SIZE Use smaller lookup tables. Instead of storing every
...
inline STRF_HD floating_decimal_64 d2d(const uint64_t ieeeMantissa, const uint32_t ieeeExponent) {
  ...
  if (ieeeExponent == 0) {
    ...
    e2 = 1 - STRF_RYU_DOUBLE_BIAS - STRF_RYU_DOUBLE_MANTISSA_BITS - 2;
    m2 = ieeeMantissa;
  } else {
    e2 = (int32_t) ieeeExponent - STRF_RYU_DOUBLE_BIAS - STRF_RYU_DOUBLE_MANTISSA_BITS - 2;
    m2 = (1ull << STRF_RYU_DOUBLE_MANTISSA_BITS) | ieeeMantissa;
  }
  ...
  if (e2 >= 0) {
    ...
    e10 = (int32_t) q;
    const int32_t k = STRF_RYU_DOUBLE_POW5_INV_BITCOUNT + pow5bits((int32_t) q) - 1;
    ...
    const int32_t k = pow5bits(i) - STRF_RYU_DOUBLE_POW5_BITCOUNT;
...
STRF_HD int d2s_buffered_n(double f, char* result) {
  ...
  const bool ieeeSign = ((bits >> (STRF_RYU_DOUBLE_MANTISSA_BITS + STRF_RYU_DOUBLE_EXPONENT_BITS)) & 1) != 0;
  const uint64_t ieeeMantissa = bits & ((1ull << STRF_RYU_DOUBLE_MANTISSA_BITS) - 1);
  const uint32_t ieeeExponent = (uint32_t) ((bits >> STRF_RYU_DOUBLE_MANTISSA_BITS) & ((1u << STRF_RYU_DOUBLE_EXPONENT_BITS) - 1));
  // Case distinction; exit early for the easy cases.
  if (ieeeExponent == ((1u << STRF_RYU_DOUBLE_EXPONENT_BITS) - 1u) || (ieeeExponent == 0 && ieeeMantissa == 0)) {
...
#endif // STRF_DETAIL_RYU_DOUBLE_HPP_INCLUDED
