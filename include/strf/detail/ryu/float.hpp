...
#define STRF_FLOAT_MANTISSA_BITS 23
#define STRF_FLOAT_EXPONENT_BITS 8
#define STRF_FLOAT_BIAS 127

// This table is generated by PrintFloatLookupTable.
#define STRF_FLOAT_POW5_INV_BITCOUNT 59
inline STRF_HD uint64_t FLOAT_POW5_INV_SPLIT(std::size_t index)
{
...
#define STRF_FLOAT_POW5_BITCOUNT 61
inline STRF_HD uint64_t FLOAT_POW5_SPLIT(std::size_t index)
{
...
inline STRF_HD bool multipleOfPowerOf5(const uint32_t value, const uint32_t p) {
  return pow5Factor(value) >= p;
}

// Returns true if value is divisible by 2^p.
inline STRF_HD bool multipleOfPowerOf2(const uint32_t value, const uint32_t p) {
...
inline STRF_HD uint32_t mulPow5InvDivPow2(const uint32_t m, const uint32_t q, const int32_t j) {
  return mulShift(m, FLOAT_POW5_INV_SPLIT(q), j);
}

inline STRF_HD uint32_t mulPow5divPow2(const uint32_t m, const uint32_t i, const int32_t j) {
...
inline STRF_HD floating_decimal_32 f2d(const uint32_t ieeeMantissa, const uint32_t ieeeExponent) {
  int32_t e2;
  uint32_t m2;
  if (ieeeExponent == 0) {
    // We subtract 2 so that the bounds computation has 2 additional bits.
    e2 = 1 - STRF_FLOAT_BIAS - STRF_FLOAT_MANTISSA_BITS - 2;
    m2 = ieeeMantissa;
  } else {
    e2 = (int32_t) ieeeExponent - STRF_FLOAT_BIAS - STRF_FLOAT_MANTISSA_BITS - 2;
    m2 = (1u << STRF_FLOAT_MANTISSA_BITS) | ieeeMantissa;
  }
  ...
  if (e2 >= 0) {
    ...
    e10 = (int32_t) q;
    const int32_t k = STRF_FLOAT_POW5_INV_BITCOUNT + pow5bits((int32_t) q) - 1;
    ...
    if (q != 0 && (vp - 1) / 10 <= vm / 10) {
      ...
      const int32_t l = STRF_FLOAT_POW5_INV_BITCOUNT + pow5bits((int32_t) (q - 1)) - 1;
    ...
    const int32_t k = pow5bits(i) - STRF_FLOAT_POW5_BITCOUNT;
...
#endif
    if (q != 0 && (vp - 1) / 10 <= vm / 10) {
      j = (int32_t) q - 1 - (pow5bits(i + 1) - STRF_FLOAT_POW5_BITCOUNT);
...
inline STRF_HD int to_chars(const floating_decimal_32 v, const bool sign, char* const result) {
...
int f2s_buffered_n(float f, char* result) {
  ...
  const bool ieeeSign = ((bits >> (STRF_FLOAT_MANTISSA_BITS + STRF_FLOAT_EXPONENT_BITS)) & 1) != 0;
  const uint32_t ieeeMantissa = bits & ((1u << STRF_FLOAT_MANTISSA_BITS) - 1);
  const uint32_t ieeeExponent = (bits >> STRF_FLOAT_MANTISSA_BITS) & ((1u << STRF_FLOAT_EXPONENT_BITS) - 1);

  // Case distinction; exit early for the easy cases.
  if (ieeeExponent == ((1u << STRF_FLOAT_EXPONENT_BITS) - 1u) || (ieeeExponent == 0 && ieeeMantissa == 0)) {
...
STRF_HD void f2s_buffered(float f, char* result) {
...
#endif // STRF_DETAIL_RYU_FLOAT_HPP_INCLUDED
