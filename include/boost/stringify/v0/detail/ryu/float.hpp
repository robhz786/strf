...
#define BOOST_STRINGIFY_V0_FLOAT_POW5_INV_BITCOUNT 59
...
#define BOOST_STRINGIFY_V0_FLOAT_POW5_BITCOUNT 61
...
inline floating_decimal_32 f2d(const uint32_t ieeeMantissa, const uint32_t ieeeExponent) {
  ...
  if (ieeeExponent == 0) {
    ...
    e2 = 1 - BOOST_STRINGIFY_V0_FLOAT_BIAS - BOOST_STRINGIFY_V0_FLOAT_MANTISSA_BITS - 2;
    m2 = ieeeMantissa;
  } else {
    e2 = (int32_t) ieeeExponent - BOOST_STRINGIFY_V0_FLOAT_BIAS - BOOST_STRINGIFY_V0_FLOAT_MANTISSA_BITS - 2;
    m2 = (1u << BOOST_STRINGIFY_V0_FLOAT_MANTISSA_BITS) | ieeeMantissa;
  }
  ...
  if (e2 >= 0) {
    ...
    e10 = (int32_t) q;
    const int32_t k = BOOST_STRINGIFY_V0_FLOAT_POW5_INV_BITCOUNT + pow5bits((int32_t) q) - 1;
    ...
    if (q != 0 && (vp - 1) / 10 <= vm / 10) {
      ...
      const int32_t l = BOOST_STRINGIFY_V0_FLOAT_POW5_INV_BITCOUNT + pow5bits((int32_t) (q - 1)) - 1;
    ...
    const int32_t k = pow5bits(i) - BOOST_STRINGIFY_V0_FLOAT_POW5_BITCOUNT;
...
#endif
    if (q != 0 && (vp - 1) / 10 <= vm / 10) {
      j = (int32_t) q - 1 - (pow5bits(i + 1) - BOOST_STRINGIFY_V0_FLOAT_POW5_BITCOUNT);
...
int f2s_buffered_n(float f, char* result) {
  ...
  const bool ieeeSign = ((bits >> (BOOST_STRINGIFY_V0_FLOAT_MANTISSA_BITS + BOOST_STRINGIFY_V0_FLOAT_EXPONENT_BITS)) & 1) != 0;
  const uint32_t ieeeMantissa = bits & ((1u << BOOST_STRINGIFY_V0_FLOAT_MANTISSA_BITS) - 1);
  const uint32_t ieeeExponent = (bits >> BOOST_STRINGIFY_V0_FLOAT_MANTISSA_BITS) & ((1u << BOOST_STRINGIFY_V0_FLOAT_EXPONENT_BITS) - 1);

  // Case distinction; exit early for the easy cases.
  if (ieeeExponent == ((1u << BOOST_STRINGIFY_V0_FLOAT_EXPONENT_BITS) - 1u) || (ieeeExponent == 0 && ieeeMantissa == 0)) {
