...
////
specified by the `<<strf_hpp#PrintableTraits_formatters,formatters>>`
type alias defined in the __PrintableTraits__ class,
and we can't change them ( neither can you change
`<<strf_hpp#PrintableTraits_forwarded_type,forwarded_type>>`
nor `<<strf_hpp#PrintableTraits_override_tag,override_tag>>` ).
...
They are specified by the `<<strf_hpp#PrintableTraits_formatters,formatters>>`
type alias defined in the __PrintableTraits__ class,
...
////
The __PrintableTraits__ type requirement specifies some
member types aliases: `<<strf_hpp#PrintableTraits_formatters,formatters>>`,
`<<strf_hpp#PrintableTraits_forwarded_type,forwarded_type>>`
and `<<strf_hpp#PrintableTraits_override_tag,override_tag>>`
These cannot be changed in our `italian_bool_facet` facet:
whatever is defined in `{printable_traits_of}<bool>`, still applies here.
...
////
In the __PrintableTraits__ requirements, you can see that there are some
member types aliases: `<<strf_hpp#PrintableTraits_formatters,formatters>>`,
`<<strf_hpp#PrintableTraits_forwarded_type,forwarded_type>>`
and `<<strf_hpp#PrintableTraits_override_tag,override_tag>>`
These cannot be changed in our `italian_bool_facet` facet:
...
`strf::{printable_traits_of}<X>::<<strf_hpp#PrintableTraits_override_tag,override_tag>>`,
...
does not have the member type alias `<<strf_hpp#PrintableTraits_override_tag,override_tag>>`,
