...
constexpr unsigned print_dest_log2_buffer_space = ...;

constexpr std::size_t print_dest_min_buffer_size =
    (std::size_t)1 << print_dest_log2_buffer_space;

template <typename CharT>
using print_dest = {destination}<CharT, print_dest_log2_buffer_space>;
}
----

* `print_dest_log2_buffer_space` is an implementation-defined value
greater than or equal to `6`.

* `print_dest_min_buffer_size` is greater than or equal to `64`. This means
