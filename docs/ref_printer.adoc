...
:print_traits:     <<print_traits,print_traits>>
...
[source,cpp,subs=normal]
----
{print_traits_of}<Arg>::<<PrintTraits_functions,make_input>>({tag}<CharT>{}, preview, facets, arg);
----
...
[source,cpp,subs=normal]
----
{print_traits_of}<Arg>::make_input({tag}<CharT>{}, preview, facets, arg)
----
