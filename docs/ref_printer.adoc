...
* `CharT`, a character type
* `Preview`, an instance of the `{pre_printing}` class template
...
[source,cpp,subs=normal]
----
std::size_t s = x.preview.<<size_accumulator_accumulated_size,accumulated_size>>();
typename T::printer_type p{x};
std::size_t predicted_size = x.preview.<<size_accumulator_accumulated_size,accumulated_size>>() - s;
...
----
statisfies the following postconditions when `Preview::<<pre_printing,size_required>>` is `true`:
...
* `Preview` is an instance of the `{pre_printing}` class template.
////
...
class pre_printing
    ...
    , public width_preview<static_cast<bool>(PrecalcWidth)>
{
public:
...
using no_pre_printing = pre_printing<precalc_size::no, precalc_width::no>;
using print_size_and_width_preview = pre_printing< precalc_size::yes
                                                 , precalc_width::yes >;
