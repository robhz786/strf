...
pre_t pre;
using printer_t = {stringifier_type}<char_type, pre_t, FPack, Arg>;
printer_t p{ {make_stringifier_input}<char_type>(pre, &#95;fpack, arg) };
static_cast<const {stringifier}<char_type>&>(p).print_to(dest);
----
...
typename DestinationCreator::destination_type dest{&#95;dest&#95;creator.create()};
----
. For each `arg` in `args\...`, creates an `stringifier` object by doing:
+
[source,cpp,subs=normal]
----
----
...
[source,cpp,subs=normal]
----
using stringifier_t = {stringifier_type}<char_type, pre_type, FPack, Arg>;
stringifier_t p{ {make_stringifier_input}<char_type>(pre, &#95;fpack, arg) };
----
. Creates/get the `{destination}` object doing
+
[source,cpp,subs=normal]
...
auto p = static_cast< const {stringifier}<CharT>& >
    ( {stringifier_type}<CharT, FPack, T>{ fpack, pre, x, {tag}<CharT>{} } )
----
...
using pre_t = {preprinting}<precalc_size::no, precalc_width::no>;
pre_t pre;
using stringifier_t = {stringifier_type}<char_type, pre_t, FPack, Arg>;
stringifier_t p{ {make_stringifier_input}<char_type>(pre, &#95;fpack, arg) };
static_cast<const {stringifier}<char_type>&>(p).print_to(dest);
----
, where `Arg` is the type in `Args\...` corresponding to `arg`
...
----
auto p = static_cast< const {stringifier}<CharT>& >
    ( {stringifier_type}<CharT, FPack, T>{fpack, pre, x, chtag} )
----
