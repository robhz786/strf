...
[source,cpp]
----
x.unsafe_transcode_size(src, src_size, surr_poli)
----
[horizontal]
...
Return type:: `std::ptrdiff_t`
Return value:: The number of character that
  `x.unsafe_transcode(dest, src, src_size, nullptr)`
  would write into `dest`, or a value a greater than that if such exact calculation is
  difficult ( but ideally not much greater ).
Precondition:: `x.transcode_func() != nullptr` is `true`
====
...
[source,cpp]
----
x.unsafe_transcode(dest, src, src_size, err_notifier)
----
Effect::
...
Return value:: A function pointer such that
               ...
               effect as `x.<<Transcoder_transcode_size,transcode_size>>(src, src_size, surr_poli)`.
====
...
Return value:: A function pointer such that
   ...
   `x.<<Transcoder_transcode,transcode>>(dest, src, src_size, err_notifier, surr_poli)`.
====
...
Return value:: A function pointer such that
               `x.unsafe_transcode_size_func() (src, src_size)` has the same
               effect as `x.<<Transcoder_unsafe_transcode_size,unsafe_transcode_size>>(src, src_size)`.
====
...
Return value:: A function pointer such that
   `x.unsafe_transcode_func() (dest, src, src_size, err_notifier)`
   has the same effect as
   `x.<<Transcoder_unsafe_transcode,unsafe_transcode>>(dest, src, src_size, err_notifier)`.
====
...
* `pos` is the greatest value not greater than `src_size` such that
   `x.count_codepoints_fast(src, pos, std::numeric_limits<std::ptrdiff_t>::max()).count`
   is equal to `c`.
...
* `pos` is the greatest value not greater than `src_size` such that
  `x.count_codepoints(src, pos, std::numeric_limits<std::ptrdiff_t>::max(), surr_poli).count`
   is equal to `c`.
Posconditions::
* `c \<= max_count` is `true`
* `pos \<= src_size` is `true`
...
----
Equivalent to `decode_encode(to_u32, from_32, src.data(), src.size(), dst, notifier, poli)`
====
...
----
decode_encode
    ( src_charset, dst_charset, src.data(), src.size()
...
decode_encode<SrcCharsetTmpl, DstCharsetTmpl>
    (src.data(), src.size(), dst, err_notifier, poli )
----
====
...
Same as:
----
decode_encode_size(to_32, size_calc_func, src.data(), src.size(), poli)
----
====
...
----
decode_encode_size
    ( src_charset, dst_charset, src.data(), src.size(), poli )
----
Compile-time requirements::
- `SrcCharset` and `DstCharset` satisfy __{Charset}__
- `SrcCharset::code_unit` is the same type as `SrcCharT`
...
----
decode_encode_size
    ( SrcCharsetTmpl<SrcCharT>(), DstCharset(), src, src_size, poli )
----
Compile-time requirements::
- `SrcCharsetTmpl<SrcCharT>` is an instance of `{static_charset}`
- `SrcCharset::code_unit` is the same type as `SrcCharT`
...
Same as:
----
decode_encode_size<SrcCharsetTmpl, DstCharset>(src.data(), src.size(), poli)
----
Compile-time requirements::
- `SrcCharsetTmpl<SrcCharT>` is an instance of `{static_charset}`
- `SrcCharset::code_unit` is the same type as `SrcCharT`
...
----
Equivalent to `unsafe_decode_encode(to_u32, from_32, src.data(), src.size(), dst, notifier)`
====
...
unsafe_decode_encode<SrcCharsetTmpl, DstCharsetTmpl>
    (src.data(), src.size(), dst, err_notifier )
----
====
...
Same as:
----
unsafe_decode_encode_size(to_32, size_calc_func, src.data(), src.size())
----
====
...
----
unsafe_decode_encode_size
    ( SrcCharsetTmpl<SrcCharT>(), DstCharset(), src, src_size )
----
Compile-time requirements::
- `SrcCharsetTmpl<SrcCharT>` is an instance of `{static_charset}`
- `SrcCharset::code_unit` is the same type as `SrcCharT`
...
Same as:
----
unsafe_decode_encode_size<SrcCharsetTmpl, DstCharset>(src.data(), src.size())
----
Compile-time requirements::
- `SrcCharsetTmpl<SrcCharT>` is an instance of `{static_charset}`
- `SrcCharset::code_unit` is the same type as `SrcCharT`
...
return {decode_encode_size} (src_charset, dst_charset, src, src_size, poli);
----
...
Same as
----
transcode_size(src_charset, dst_charset, src.data(), src.size(), poli)
----
...
Same as
----
transcode_size <SrcCharsetTmpl, DstCharset>(src.data(), src.size(), poli)
----
...
return {unsafe_decode_encode_size} (src_charset, dst_charset, src, src_size);
----
...
Same as
----
unsafe_transcode_size(src_charset, dst_charset, src.data(), src.size())
----
...
Same as
----
unsafe_transcode_size(SrcCharsetTmpl<SrcCharT>(), DstCharset(), src, src_size)
----
...
Same as
----
unsafe_transcode_size <SrcCharsetTmpl, DstCharset>(src.data(), src.size())
----
