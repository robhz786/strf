...
template <typename CharT>
using transcode_dest = {destination}<CharT, 3>;
...
[horizontal]
Postcondition:: `dest.<<destination_hpp#destination_recycle_buffer,recycle_buffer>>()` is not called if
`dest.<<destination_hpp#destination_buffer_space,buffer_space>>()` is not less then the value returned by
