...
:count_codepoints_result: <<charset_function_pointers,count_codepoints_result>>
...
using transcode_size_f = std::ptrdiff_t ({asterisk})
    ( const SrcCharT{asterisk} src
    , const SrcCharT{asterisk} src_end
    , {surrogate_policy} surr_poli );

template <typename SrcCharT, typename DestCharT>
using unsafe_transcode_f = void ({asterisk})
    ( {transcode_dest}<DestCharT>& dest
    , const SrcCharT{asterisk} src
    , const SrcCharT{asterisk} src_end
    , {transcoding_error_notifier}{asterisk} err_notifier );
...
template <typename SrcCharT>
using unsafe_transcode_size_f = std::ptrdiff_t ({asterisk})
    ( const SrcCharT{asterisk} src
    , const SrcCharT{asterisk} src_end );
...
[source,cpp]
----
x.transcode_size(src, src_end, surr_poli)
----
[horizontal]
Return type:: `std::ptrdiff_t`
Return value:: The number of character that
  `x.transcode(dest, src, src_end, nullptr, surr_poli)`
...
[source,cpp]
----
x.transcode(dest, src, src_end, err_notifier, surr_poli)
----
[horizontal]
...
+
Each codepoint in `src` that is not supported by the destination
character encoding is translated to the replacement character ( that is returned by
`dest_sc.<<Charset_replacement_char, replacement_char()>>`,
where `dest_sc` represents the destination encoding ).
Each time that happens,
...
[source,cpp]
----
x.unsafe_transcode_size(src, src_end, surr_poli)
----
[horizontal]
Precondition:: The behavior is undefined if the content of `src` has any
               invalid-sequence, __i.e.__, if it is non-conformant to the
               source character encoding.

Return type:: `std::ptrdiff_t`
Return value:: The number of character that
  `x.unsafe_transcode(dest, src, src_end, nullptr)`
  would write into `dest`, or a value a greater than that if such exact calculation is
  difficult ( but ideally not much greater ).
Precondition:: `x.transcode_func() != nullptr` is `true`
====
...
[source,cpp]
----
x.unsafe_transcode(dest, src, src_end, err_notifier)
----
Effect::

Converts the content of `src` from one encoding to another writing
the result to `dest`.
The behavior is undefined if the contnet of `src` has any
invalid-sequence, __i.e.__, if it is
non-conformant to the source character encoding.
+
As it happens in `<<Transcoder_transcode,transcode>>` function,
each codepoint in `src` that is not supported by the destination
character encoding is translated to the replacement character ( that is returned by
`dest_sc.<<Charset_replacement_char, replacement_char()>>`,
where `dest_sc` represents the destination encoding ).
...
unsupported codepoint ) is not `0xFFFD`.
+
Contrary to `<<Transcoder_transcode,transcode>>`, this function
does not have `{surrogate_policy}` parameter, since the policy is
assumed to be `surrogate_policy::strict`.
====
...
Return value:: A function pointer such that
   `x.transcode_func() (dest, src, src_end, err_notifier, surr_poli)`
   has the same effect as
   `x.<<Transcoder_transcode,transcode>>(dest, src, src_end, err_notifier, surr_poli)`.
====
...
Return value:: A function pointer such that
   `x.unsafe_transcode_func() (dest, src, src_end, err_notifier)`
   has the same effect as
   `x.<<Transcoder_unsafe_transcode,unsafe_transcode>>(dest, src, src_end, err_notifier)`.
====
...
----
[horizontal]
Effect:: Calls `transcode_func()(dest, src, src_end, err_notifier, surr_poli)`
====
====
[source,cpp,subs=normal]
----
...
----
[horizontal]
Effect:: Calls `transcode_size_func()(src, src_end, surr_poli)`
====
...
They all guarantee not to call `dst.recycle()` if `dst.space()`
...
void decode_encode
    ...
    , {surrogate_policy} poli = surrogate_policy::strict );

}
----

Effect:: Converts the content in `src` to UTF-32 using `to_u32`,
...
decode_encode<SrcCharsetTmpl, DstCharsetTmpl>
    (src.data(), src.data() + src.size(), dst, err_notifier, poli )
----
====
...
template <typename SrcCharT>
std::ptrdiff_t decode_encode_size
    ( {transcode_f}<SrcCharT, char32_t> to_u32
...
----
Effect::
Convert the content of `src` to UTF-32 using `to_u32`, and return the value
returned by `size_calc_func(u32buff, u32buff_size, poli)`, where
`u32buff` is the content written by `to_u32`, and `u32buffsize` is
its size.
====
...
Same as:
----
decode_encode_size(to_32, size_calc_func, src.data(), src.data() + src.size(), poli)
----
====
...
----
decode_encode_size
    ( SrcCharsetTmpl<SrcCharT>(), DstCharset(), src, src_end, poli )
----
Compile-time requirements::
- `SrcCharsetTmpl<SrcCharT>` is an instance of `{static_charset}`
- `SrcCharset::code_unit` is the same type as `SrcCharT`
...
Same as:
----
decode_encode_size<SrcCharsetTmpl, DstCharset>(src.data(), src.data() + src.size(), poli)
----
Compile-time requirements::
- `SrcCharsetTmpl<SrcCharT>` is an instance of `{static_charset}`
- `SrcCharset::code_unit` is the same type as `SrcCharT`
...
Equivalent to
----
unsafe_decode_encode(to_u32, from_32, src.data(), src.data() + src.size(), dst, notifier)
----
====
...
unsafe_decode_encode<SrcCharsetTmpl, DstCharsetTmpl>
    (src.data(), src.data() + src.size(), dst, err_notifier )
----
====
...
template <typename SrcCharT>
std::ptrdiff_t unsafe_decode_encode_size
    ( {unsafe_transcode_f}<SrcCharT, char32_t> to_u32
...
Effect::
Convert the content of `src` to UTF-32 using `to_u32`, and return the value
returned by `size_calc_func(u32buff, u32buff_size)`, where
`u32buff` is the content written by `to_u32`, and `u32buffsize` is
its size.
====
...
Same as:
----
unsafe_decode_encode_size(to_32, size_calc_func, src.data(), src.data() + src.size())
----
====
...
----
unsafe_decode_encode_size
    ( SrcCharsetTmpl<SrcCharT>(), DstCharset(), src, src_end )
----
Compile-time requirements::
- `SrcCharsetTmpl<SrcCharT>` is an instance of `{static_charset}`
- `SrcCharset::code_unit` is the same type as `SrcCharT`
...
Same as:
----
unsafe_decode_encode_size<SrcCharsetTmpl, DstCharset>(src.data(), src.data() + src.size())
----
Compile-time requirements::
- `SrcCharsetTmpl<SrcCharT>` is an instance of `{static_charset}`
- `SrcCharset::code_unit` is the same type as `SrcCharT`
...
if (func != nullptr)
    return func(dst, src, src_end, err_notifier, poli);
else
...
return {decode_encode_size} (src_charset, dst_charset, src, src_end, poli);
----
...
Same as
----
transcode_size(src_charset, dst_charset, src.data(), src.data() + src.size(), poli)
----
...
Same as
----
transcode_size <SrcCharsetTmpl, DstCharset>(src.data(), src.data() + src.size(), poli)
----
...
if (func != nullptr)
    return func(dst, src, src_end, err_notifier);
else
...
return {unsafe_decode_encode_size} (src_charset, dst_charset, src, src_end);
----
...
Same as
----
unsafe_transcode_size(src_charset, dst_charset, src.data(), src.data() + src.size())
----
...
Same as
----
unsafe_transcode_size(SrcCharsetTmpl<SrcCharT>(), DstCharset(), src, src_end)
----
...
template < template <class> class SrcCharsetTmpl
         , typename DstCharset
         , typename SrcCharT >
std::ptrdiff_t unsafe_transcode_size ( std::basic_string_view<SrcCharT> src );
...
Same as
----
unsafe_transcode_size <SrcCharsetTmpl, DstCharset>(src.data(), src.data() + src.size())
----
