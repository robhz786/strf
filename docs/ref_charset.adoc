...
constexpr surrogate_policy to_surrogate_policy(transcode_flags f) noexcept;
...
* `transcode_stop_reason::bad_destination` implies that the transcoding stopped
  because the destination ( which is a `transcode_dest` object ) is in "bad"
  state (__i.e.__ `<<destination_hpp#output_buffer_good,good>>()` returned `false`)
  or it became in bad state after a call to
  `<<destination_hpp#output_buffer_recycle,recycle>>()`.
...
template <typename SrcCharT, typename DestCharT>
using unsafe_transcode_f = transcode_f<SrcCharT, DestCharT>
...
template <typename SrcCharT>
using unsafe_transcode_size_f = strf::transcode_size_f<SrcCharT, DestCharT>;
...
[source,cpp]
----
x.transcode_size(src, src_end, flags)
----
Precondition:: `x.transcode_func() != nullptr` is `true`
Return type:: `{transcode_size_result}`
Return value:: A value `res` such that
+
* `res.**size**` is the number of character that `x.transcode(src, src_end, dest, nullptr, flags)`
           would write into `dest`, assuming that `dest` would be in
           `<<destination_hpp#output_buffer_good,good state>>` during all trancoding.
* `res.**stop_reason**` is the same `{transcode_stop_reason}` value returned by
           `x.transcode(src, src_end, dest, nullptr, flags)`,
            assuming that `dest` would not be in
           `<<destination_hpp#output_buffer_good,good state>>` during all trancoding.
* `res.**ptr**` points immediately after the last character in the range `[src, src_end)`
...
[source,cpp]
----
x.transcode(src, src_end, dest, err_notifier, flags)
----
Precondition:: `x.transcode_func() != nullptr` is `true`
Postconditions:: `dest.recycle()` is not called() if
        `dest.buffer_space() >= x.transcode_size(src, src_end, surr_poli)` is `true`.
...
* `count` is number of code units in the invalid sequence
* `ch_size` is `sizeof(SrcCharT)`

After that, if `flags` has the flag `stop_on_invalid_sequence`, then the function shall
stop and return a value with `stop_reason` equal to
...
and continue the iteration after the invalid sequence.

A surrogate codepoint shall not be considered invalid
if the flag `transcode_flags::lax_surrogate_policy` is set in `flags`.

Similarly, when the function reads a codepoint that is both not supported by the destination
encoding and not U+FFFD, it calls
`err_notifier\-><<transcoding_error_notifier,unsupported_codepoint>>(dest_sc.<<Charset_name,name()>>, code)`
if `err_notifier` is not null and `code` ( which is the
unsupported codepoint ) is not U+FFFD.
...
even if it is, __i.e.__ `err_notifier\->unsupported_codepoint`
...
The function shall not write anything to `dest` if it is in bad state
(__i.e.__ the return value of `dest.good()` is `false`).
Instead, it should stop the iteration and return a value with `stop_reason`
equal to `transcode_stop_reason::bad_destination`.

`transcode_stop_reason::bad_destination` should only be returned when
there is something to be written after the `dest` became in bad state.
For instance, if the input starts with an invalid sequence and
`flags` has `stop_on_invalid_sequence` set, then the `stop_reason`
shall be `transcode_stop_reason::invalid_sequence`, even if
`dest.good()` return value is already `false` before the iteration starts.
--
...
[source,cpp]
----
x.unsafe_transcode_size(src, src_end, flags)
----
[horizontal]
Precondition::
* The input has no invalid sequence
* `x.unsafe_transcode_() != nullptr` is `true`The input has no invalid sequence
Return type:: `{transcode_size_result}`

Effect:: Exactly the same as `x.transcode_size(src, src_end, flags)`,
except that the behaviour is undefined if the input contains any
invalid sequence.
+
Note that if the flag `transcode_flags::lax_surrogate_policy` is present
in `flags`, then the transcoding of a surrogate character input
is well defined.
====
...
[source,cpp]
----
x.unsafe_transcode(src, src_end, dest, err_notifier, flags)
----
Preconditions::
...
Effect:: Exactly the same as `x.transcode_(src, src_end, err_notifier, flags)`,
except that the behaviour is undefined if the input contains any
invalid sequence.
+
Note that if the flag `transcode_flags::lax_surrogate_policy` is present
...
Return value:: A function pointer such that
               `x.transcode_size_func() (src, src_end, surr_poli)` has the same
               effect as `x.<<Transcoder_transcode_size,transcode_size>>(src, src_end, surr_poli)`.
====
...
Return value:: A function pointer such that
   `x.transcode_func() (src, src_end, dest, err_notifier, surr_poli)`
   has the same effect as
   `x.<<Transcoder_transcode,transcode>>(src, src_end, dest, err_notifier, surr_poli)`.
====
...
Return value:: A function pointer such that
               `x.unsafe_transcode_size_func() (src, src_end)` has the same
               effect as `x.<<Transcoder_unsafe_transcode_size,unsafe_transcode_size>>(src, src_end)`.
====
...
----
[horizontal]
Return type:: `{unsafe_transcode_f}<SrcCharT, DestCharT>`
Return value:: A function pointer such that
   `x.unsafe_transcode_func() (src, src_end, dest, err_notifier)`
   has the same effect as
   `x.<<Transcoder_unsafe_transcode,unsafe_transcode>>(src, src_end, dest, err_notifier)`.
====
...
* `src_end`, a value of type `const CharT{asterisk}`, such that `src <= src_end` is true
...
Return type:: `int`
Return value:: The size of the string containing the UTF-32 character `ch32`,
               if `ch32` is supported in this encoding. Otherwise, `-1`.
Postcondition:: The return value must be greater than zero.
====
====
[source,cpp]
...
[source,cpp]
----
x.count_codepoints(src, src_end, max_count, surr_poli)
----
[horizontal]
...
* `ptr` is the greatest pointer not greater than `src_end` such that
  `x.count_codepoints(src, ptr, std::numeric_limits<std::ptrdiff_t>::max(), surr_poli).count`
   is equal to `c`.
Posconditions::
* `c \<= max_count` is `true`
* `ptr \<= src_end` is `true`
...
Return type:: A __{Transcoder}__ type `T`, such that
              `T::<<Transcoder_src_char_type,src_char_type>>` is `char32_t` and
              `T::<<Transcoder_dst_char_type,dst_char_type>>` is `CharT`
...
[horizontal]
Return type:: A __{Transcoder}__ type `T`, such that
              `T::<<Transcoder_src_char_type,src_char_type>>` is `CharT` and
              `T::<<Transcoder_dst_char_type,dst_char_type>>` is `char32_t`
...
----
[horizontal]
Effect:: Calls `transcode_func()(src, src_end, dest, err_notifier, flags)`
====
====
[source,cpp,subs=normal]
----
...
----
[horizontal]
Effect:: Calls `transcode_size_func()(src, src_end, flags)`
====
...
public:
    ...
    std::ptrdiff_t count_codepoints
        ( const code_unit{asterisk} src, const code_unit{asterisk} src_end
        , std::ptrdiff_t max_count, {surrogate_policy} surr_poli ) const;
...
[horizontal]
Effect:: Calls and returns
        `this\->data\->count_codepoints_func(src, src_end, max_count, surr_poli)`.
====
====
[source,cpp,subs=normal]
----
...
( `decode_encode`, `decode_encode_size`,
  ...
  __{Transcoder}__ class ( a `static_transcoder` specialization)
  for all combination of two character encodings.
...
* `to_u32` is the value returned by
   ...
   where `src_charset` is a __{Charset}__ object that represents some
   character encoding __A__
* `from_u32` is the value returned by
   ...
   where `dst_charset` is a __{Charset}__ object that represents some
   a character encoding __B__
...
`__T__::<<Transcoder_transcode,transcode>>(src, src_end, dst, notifier, flags)`,
where `__T__` would be a __{Transcoder}__ class that transcodes from __A__ to __B__.
--
...
[source,subs=normal]
----
 __src_charset__.<<Charset_count_codepoints,count_codepoints>>(res.**stale_ptr**, src_end, res.**u32dist**, __surr_poli__).<<count_codepoints_result,ptr>>
----
+
, where:

** `__src_charset__` is a __{Charset}__ object that describes the source character encoding
** `__surr_poli__` is `<<transcode_flags,to_surrogate_policy>>(flags)`
--

Postcondition::
`dst.recycle()` is not called  if `dst.space()` is not less then the amount of character to be written in `dst`
====
...
decode_encode<SrcCharsetTmpl, DstCharsetTmpl>
    (src.data(), src.data() + src.size(), dst, err_notifier, flags )
----
====
...
`decode_encode_size` has the same effects as calling
`__T__::<<Transcoder_transcode_size,transcode_size>>(src, src_end, flags)`,
where `__T__` would be a __{Transcoder}__ class that transcodes from __A__ to __B__.
--
...
+
--
* `res.**size**` is the number of character that
           `{decode_encode}(src, src_end, dest, nullptr, flags)`
           would write into `dest`, assuming that `dest` would be in
           `<<destination_hpp#output_buffer_good,good state>>` during all trancoding.
* `res.**stop_reason**` (see `{transcode_stop_reason}`)
* `res.**stale_ptr**` is **not** the pointer past the last read element in the input range `[src, src_end)`. Instead, it points `res.**u32dist**` codepoints before that.
...
----
decode_encode_size
    ( SrcCharsetTmpl<SrcCharT>(), DstCharset(), src, src_end, flags )
----
Compile-time requirements::
- `SrcCharsetTmpl<SrcCharT>` is an instance of `{static_charset}`
- `SrcCharset::code_unit` is the same type as `SrcCharT`
...
decode_encode_size<SrcCharsetTmpl, DstCharset>
    ( src.data(), src.data() + src.size(), flags )
----
Compile-time requirements::
- `SrcCharsetTmpl<SrcCharT>` is an instance of `{static_charset}`
- `SrcCharset::code_unit` is the same type as `SrcCharT`
...
`__T__::{unsafe_transcode}(src, src_end, dst, notifier, flags)`,
where `__T__` would be a __{Transcoder}__ class that transcodes from __A__ to __B__.
--
...
Equivalent to
----
unsafe_decode_encode(to_u32, from_32, src.data(), src.data() + src.size(), dst, notifier, flags)
----
====
...
+
--
`unsafe_decode_encode_size` has the same effects as calling
`__T__::<<Transcoder_transcode_size,unsafe_transcode_size>>(src, src_end, flags)`,
where `__T__` would be a __{Transcoder}__ class that transcodes from __A__ to __B__.
--
...
=== Function template `do_transcode` [[do_transcode]]
...
template < typename SrcCharset, typename DstCharset
         , typename SrcCharT, typename DstCharT >
{decode_encode_result}<SrcCharT> do_transcode
    ( SrcCharset src_charset
    , DstCharset dst_charset
    , const SrcCharT{asterisk} src
    , const SrcCharT{asterisk} src_end
...
if (func != nullptr) {
    auto r = func(src, src_end, dst, err_notifier, flags);
    return {r.ptr, 0, r.stop_reason};
}
...
Same as:
----
do_transcode <SrcCharsetTmpl, DstCharsetTmpl>
    ( src.data(), src.data() + src.size(), dst, err_notifier, flags );
----
Compile-time requirements::
- `SrcCharsetTmpl<SrcCharT>` is an instance of `{static_charset}`
...
if (func != nullptr) {
    auto r = func(src, src_end, flags);
    return {r.size, r.ptr, 0, r.stop_reason};
}
return {decode_encode_size} (src_charset, dst_charset, src, src_end, flags);
----
...
=== Function template `do_unsafe_transcode` [[do_unsafe_transcode]]
...
if (func != nullptr) {
    auto r = func(src, src_end, dst, err_notifier);
    return {r.ptr, 0, r.stop_reason};
}
...
Same as:
----
do_unsafe_transcode <SrcCharsetTmpl, DstCharsetTmpl>
    ( src.data(), src.data() + src.size(), dst, err_notifier, flags );
----
Compile-time requirements::
- `SrcCharsetTmpl<SrcCharT>` is an instance of `{static_charset}`
...
return {unsafe_decode_encode_size} (src_charset, dst_charset, src, src_end, flags);
----
