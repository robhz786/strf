...
:invalid_seq_notifier_c: <<invalid_seq_notifier_c,invalid_seq_notifier_c>>
:invalid_seq_notifier: <<invalid_seq_notifier,invalid_seq_notifier>>
:invalid_seq_notifier_nullptr: <<invalid_seq_notifier_nullptr,invalid_seq_notifier_nullptr>>
:InvalidSeqNotifierPtr: <<InvalidSeqNotifierPtr,InvalidSeqNotifierPtr>>
...
A facet of the `invalid_seq_notifier_c` category must satisfy the
__{InvalidSeqNotifierPtr}__ type requirements.
...
struct invalid_seq_notifier_c {
    ...
    static constexpr {invalid_seq_notifier_nullptr} get_default() noexcept {
        return {};
    }
};
} // namespace strf
...
==== Type requirement __InvalidSeqNotifierPtr__ [[InvalidSeqNotifierPtr]]

Given

* `X`, a __InvalidSeqNotifierPtr__ type
* `x`, a const value of type `X`

The following must hold:

* `{facet_category}<X>` is a type alias to `invalid_seq_notifier_c`
...
----
[horizontal]
Return type:: `{invalid_seq_notifier}{asterisk}`
====

==== Struct `invalid_seq_notifier_nullptr` [[invalid_seq_notifier_nullptr]]
...
struct invalid_seq_notifier_nullptr {
    ...
    constexpr invalid_seq_notifier* get() const { return nullptr; }
};

} // namespace strf
----

==== Struct `invalid_seq_notifier_ptr` [[invalid_seq_notifier_ptr]]
...
struct invalid_seq_notifier_ptr {
    ...
    constexpr invalid_seq_notifier* get() const { return ptr; }
...
using transcode_f = void ({asterisk})
    ...
    , std::size_t src_size
    , {invalid_seq_notifier}{asterisk} inv_seq_notifier
...
* `inv_seq_notifier`, is a pointer of type `{invalid_seq_notifier}`
...
Precondition:: `x.transcode_func() != nullptr` is `true`
Postconditions:: `dest.recycle_buffer()` is not called() if
...
----
[horizontal]
Effect:: Calls `transcode_func()(dest, src, src_size, inv_seq_notifier, surr_poli)`
====
====
[source,cpp,subs=normal]
----
...
[horizontal]
Postcondition:: `dest.<<destination_hpp#output_buffer_recycle_buffer,recycle_buffer>>()` is not called if
