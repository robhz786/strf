...
[source,cpp,subs=normal]
----
__foo_print_traits__::make_input({tag}<CharT>{}, __preview__, __fp__, __foo__)
----
...
[source,cpp,subs=normal]
----
__PrintTraits__::make_input({tag}<CharT>{}, preview, fp, x)
----
, and the type of `x` is not an instance `{value_with_formatters}`, then
the library invokes the following instead:
[source,cpp,subs=normal]
----
__PrintTraits__::make_input({tag}<CharT>{}, preview, fp, strf::fmt(x))
----
