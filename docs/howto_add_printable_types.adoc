...
<2> This is usually the printable type itself or a const reference to it.
    The library uses it in some situations for argument forwarding.
    {join} objects store values of this type, instead of the printable type.
...
<4> The library will always use an instance of the `{preprinting}` class template
    as the `Pre` argument
<5> The library will always use an instance of the `{facets_pack}` class template
    as the `FPack` argument. `fp` contains the facet objects.
...
Remember that `make_printer_input` function does not actually
...
do another thing. `{usual_printer_input}` contains a `{preprinting}`
