...
:stringifier:                <<strf_hpp#stringifier,stringifier>>
:preprinting:                <<strf_hpp#preprinting,preprinting>>
...
:strf_make_stringifier_input:    <<strf_hpp#make_stringifier_input,strf::make_stringifier_input>>
:usual_stringifier_input:        <<strf_hpp#usual_stringifier_input,usual_stringifier_input>>
...
struct your_printable_type_traits { /{asterisk}\...{asterisk}/ }; // a __{PrintableTraits}__ type
...
just define the template specialization `printable_traits<your_type>`,
since this is the fallback the library uses when there is not
matching overload.

Whatever approach you use, `printable_traits<your_type>`
or the return type of `tag_invoke` must satisfy __{PrintableTraits}__.

== Creating a __{PrintableTraits}__ class [[CreatePrintableTraits]]

As an example, let's see how we can make
...
<2> `Pre` is expected to be an instance of the `{preprinting}`
class template.
...
`std::complex` values in the same way that it would handle the `{join}`
object created above ( `arg2` ). That's because the expression:

[source,cpp,subs=normal]
----
{strf_make_stringifier_input}<CharT>(__pre__, __fp__, __foo__)
----
is equivalent to
...
----
where `__foo_type_traits__` is the return type of
`tag_invoke(strf::printable_tag{}, __foo__)`.

( Actually, `{strf_make_stringifier_input}` may call something else
...
This would happen, for instance, if the `join` in the snippet above
contained a pointer to any local variable.
Remember that `make_stringifier_input` function does not actually
print the value, it just creates and returns an object.

Anyway, sometimes it is not possible to just return
`strf::make_stringified_input(__something__...)`,
__i.e.__ to simply convert the value to another type that is already printable.
A <<creating_stringifier,further section>> explains how to deal with such cases.
...
if (form != complex_form::polar) {
    auto j1 = strf::join
            ...
            , v.second
            , static_cast<CharT>(')') );
    return strf::make_stringifier_input<CharT>(pre, fp, j1);
}
auto j2 = strf::join
    ( v.first
    , strf::{conv}(middle_string(form), strf::{utf}<char16_t>)
    , v.second );
return strf::make_stringifier_input<CharT>(pre, fp, j2); // different return type !
----
...
`{value_with_formatters}<printable_traits<std::complex<...>>, Fmts\...>`, where `Fmts\...`
...
struct printable_traits<std::complex<FloatT>> {
        ...
        , strf::{value_with_formatters}<T\...> arg )
    {
        ...
        return strf::make_stringifier_input<CharT>(pre, fp, arg3);
    }
};
...
== The low-level way [[creating_stringifier]]

Sometimes, when creating a __PrintableTraits__ class,
it is not possible possible to make its `make_input`
member function just return `strf::make_stringifier_input(__something__..)`
...
The return type of `make_input` must aways be a __{StringifierInput}__
type, and the `{usual_stringifier_input}` class template is syntatic sugar to
achieve that.
...
class that derives from `{stringifier}<CharT>`, or that is
https://en.cppreference.com/w/cpp/language/cast_operator[convertible]
to `const {stringifier}<CharT>&`. It must also be constructible from the
...
public:
    ...
    template <typename\... T>
    explicit std_complex_printer(strf::usual_stringifier_input<T\...>);
...
do another thing. `{usual_stringifier_input}` contains a `{preprinting}`
reference, named `pre`. When the first template argument of this
`preprinting` is `precalc_size::yes`, then
...
static_assert(! pre_t::<<strf_hpp#preprinting,size_required>>);
----
...
`preprinting` is `precalc_width::yes`, then we must inform
...
----
using pre_t = typename strf::usual_stringifier_input<T...>::preprinting_type;
static_assert(! pre_t::<<strf_hpp#preprinting,width_required>>);

// or, if you don't want to calculate the size either:
static_assert(pre_t::<<strf_hpp#preprinting,nothing_required>>);
----
...
`{pre}`. The rest of the content we can easily calculate manually:

////
For the floating point values, we can use the global function template
`{pre}`, which we actually could use for the whole content . I mean, I could
implement the `pre_` member function very similiarly to `print_to`,
basically just replacing every `print(__args__...)` by
`strf::precalculate(pp, facets, __args__...)`. But there is a more efficient
implementation:
////
...
void std_complex_printer<CharT, FloatT>::pre_(Pre& pp, const WidthCalc& wcalc) const
{
...
another stringifier &#x2014; you could name `fmt_std_complex_printer`
