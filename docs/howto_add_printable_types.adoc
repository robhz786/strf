...
[source,cpp,subs=normal]
----
__PrintableDef__::make_printer({tag}<CharT>{}, pre, facets, x)
----
, and the type of `x` is not an instance `{value_and_format}`, then
the library tries the following instead:
[source,cpp,subs=normal]
----
__PrintableDef__::make_printer({tag}<CharT>{}, pre, facets, strf::fmt(x))
----
====
...
struct strf::printable_def<std::complex<FloatT>> {
        ...
        , std::complex<FloatT> arg)
    {
        to(dst) .with(facets)
            ( (CharT)'(', arg.real()
            , strf::unsafe_transcode(", ", strf::ascii<char>)
            , arg.imag(), (CharT)')');
    }
    // \...
};
