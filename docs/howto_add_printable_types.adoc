...
`pre_printing` is `preview_size::yes`, then
...
static_assert(! preview_type::<<strf_hpp#pre_printing,size_required>>);
----
...
`pre_printing` is `preview_width::yes`, then we must inform
...
----
using preview_type = typename strf::usual_arg_printer_input<T...>::preview_type;
static_assert(! preview_type::<<strf_hpp#pre_printing,width_required>>);

// or, if you don't want to preview the size either:
static_assert(preview_type::<<strf_hpp#pre_printing,nothing_required>>);
----
