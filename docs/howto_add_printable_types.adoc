...
:pre_printing:               <<strf_hpp#pre_printing,pre_printing>>
...
<2> `Pre` is expected to be an instance of the `{pre_printing}`
class template.
...
[source,cpp,subs=normal]
----
{strf_make_arg_printer_input}<CharT>(__pre__, __fp__, __foo__)
----
is equivalent to:
[source,cpp,subs=normal]
----
...
if (form != complex_form::polar) {
    auto j1 = strf::join
            ...
            , v.second
            , static_cast<CharT>(')') );
    return strf::make_arg_printer_input<CharT>(pre, fp, j1);
}
auto j2 = strf::join
    ( v.first
    , strf::{conv}(middle_string(form), strf::{utf}<char16_t>)
    , v.second );
return strf::make_arg_printer_input<CharT>(pre, fp, j2); // different return type !
----
...
struct printing_traits<std::complex<FloatT>> {
        ...
        , strf::{value_with_formatters}<T\...> arg )
    {
        ...
        return strf::make_arg_printer_input<CharT>(pre, fp, arg3);
    }
};
...
do another thing. `{usual_arg_printer_input}` contains a `{pre_printing}`
reference, named `pre`. When the first template argument of this
`pre_printing` is `precalc_size::yes`, then
...
static_assert(! pre_t::<<strf_hpp#pre_printing,size_required>>);
----
...
`pre_printing` is `precalc_width::yes`, then we must inform
...
----
using pre_t = typename strf::usual_arg_printer_input<T...>::pre_printing_type;
static_assert(! pre_t::<<strf_hpp#pre_printing,width_required>>);

// or, if you don't want to calculate the size either:
static_assert(pre_t::<<strf_hpp#pre_printing,nothing_required>>);
----
