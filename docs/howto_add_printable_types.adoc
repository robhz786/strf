...
----
<1> `Preview` is expected to be an instance of the `{print_preview}`
class template ( but you only really need care to about it 
when <<creating_printer,creating a printer class>> )
<2> `FPack` is expected to be an instance of the `{facets_pack}` class template.
This arguments contains the facet objects.
<3> The third argument passed to `make_printer_input` is the value to be printed.
...
Well, actually, sometimes it is instead equivalent to:
[source,cpp,subs=normal]
----
...
To make them printable we to create a new `make_printer_input` member function:
...
[source,cpp,subs=normal]
----
__PrintTraits__::template make_printer_input<CharT>(preview, fp, x)
----
, and the type of `x` is not an instance `{value_with_formatters}`, then
the library invokes the following instead:
[source,cpp,subs=normal]
----
__PrintTraits__::template make_printer_input<CharT>(preview, fp, strf::fmt(x))
----
