...
public:
    ...
    {width_t} <<width_by_func_char_width,char_width>>
        ( Charset cs
        , typename Charset::char_type ) const;
...
----
[horizontal]
Return value:: `func_(cs.<<Charset_decode_char,decode_char>>(ch))`
====
...
Return value::: The sum of the values of `func_(ch32)`, where `ch32` is
   ...
   an arbitrary object whose type derives from `{basic_outbuff}<char32_t>` -- by the call
+
[source,cpp,subs=normal]
----
