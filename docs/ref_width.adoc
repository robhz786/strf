...
Return value::: The sum of the values of `func_(ch32)`, where `ch32` is
   each of the `char32_t` values that would be written into `dest` --
   an arbitrary object whose type derives from `{transcode_dest}<char32_t>` -- by the call
+
[source,cpp,subs=normal]
----
cs.<<Charset_to_u32,to_u32>>().<<Transcoder_transcode,transcode>>
    ( dest, str, str_len, {invalid_seq_notifier}{}, surr_poli )
----
...
* `ret.width` is the sum of the values of `func_(ch32)`, where `ch32` is
   each of the `char32_t` values that are written into `dest` by the call
+
[source,cpp,subs=normal]
----
cs.<<Charset_to_u32,to_u32>>().<<Transcoder_transcode,transcode>>
    ( dest, str, ret.pos, {invalid_seq_notifier}{}, surr_poli )
----
, where `dest` is any object whose type derives from `{transcode_dest}<char32_t>`.
* `ret.pos` is the greatest value that is less than or equal to `str_len`
