...
[source,cpp,subs=normal]
----
using pre_t = {pre_printing}<preview_size::no, preview_width::no>;
pre_t pre;
...
. Creates an object `pre` of type
  `{pre_printing}<preview_size::yes, preview_width::no>`.
+
[source,cpp,subs=normal]
----
using pre_type = {pre_printing}<preview_size::yes, preview_width::no>;
pre_type pre;
----
. For each `arg` in `args\...`, creates an `arg_printer` object by doing:
+
...
[source,cpp,subs=normal]
----
pre_printing<preview_size::yes, preview_width::no> pre;
...
----
, where `pre` is an instance of `pre_printing<preview_size::no, preview_width::no>`,
