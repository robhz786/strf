...
[source,cpp,subs=normal]
----
using pre_t = {pre_printing}<precalc_size::no, precalc_width::no>;
pre_t pre;
...
. Creates an object `pre` of type
  `{pre_printing}<precalc_size::yes, precalc_width::no>`.
+
[source,cpp,subs=normal]
----
using pre_type = {pre_printing}<precalc_size::yes, precalc_width::no>;
pre_type pre;
----
. For each `arg` in `args\...`, creates an `arg_printer` object by doing:
+
...
dest_type dest{&#95;dest&#95;creator.create(pre.<<size_preview_accumulated_size,accumulated_size>>())};
----
...
[source,cpp,subs=normal]
----
pre_printing<precalc_size::yes, precalc_width::no> pre;
...
----
, where `pre` is an instance of `pre_printing<precalc_size::no, precalc_width::no>`,
