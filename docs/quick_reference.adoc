...
:charset_c:       <<strf_hpp.html#charset_c,charset_c>>
...
Note:: You can see there is no equivalent to `std::internal` in the above table.
       Instead, look for the `pad0` format function if you want to pad zeros
       when printing integers or floating-points.
...
| Transcodes the input string if `cs` is different from the
  ...
  ( The output charset is defined by the facet value
    of the `<<facets,charset_c>><CharOut>` facet category,
...
| If `cs` is different from the ouput charset, transcodes the
  ...
  facet object is not null ( see section
  "<<transcoding_error_notifier,Transcoding error handling>>")
