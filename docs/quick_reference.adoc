...
* The `<<strf_hpp#width_as_u32len,width_as_u32len>>` facet value also
  ...
  https://en.wikipedia.org/wiki/Specials_(Unicode_block)#Replacement_character[replacement character &#xFFFD;] ). This is the default.
+
.Example
[source,cpp]
----
...
----
+
* The `<<strf_hpp#std_width_calc,std_width_calc>>` calculates the width just
as http://eel.is/c++draft/format.string.std#11[specified] to `std::format`,
which means it takes into consideration the presence of
https://unicode.org/reports/tr29/[grapheme clusters] and differentiates
half-width and full-width codepoints. It doesn't require however the string
to be encoded in UTF.
+
