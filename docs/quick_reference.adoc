...
----
[horizontal]
Return type:: `destination<__CharT__>&`
Return value:: `__destination_ref__`
Supports reserve:: No
...
//-
Return type:: `<<destination_hpp#basic_cstr_writer,basic_cstr_writer<__CharT__>::result`>>
...
* `__end__ >= __char_ptr__`
//-
Return type:: `<<destination_hpp#basic_char_array_writer,basic_char_array_writer<__CharT__>::result`>>
Return value:: a value `r`, such that:
...
| `<<destination_hpp#basic_cstr_writer, basic_cstr_writer>><__CharT__>`
| Writes C strings. Always writes the termination character `'\0'`.

| `<<destination_hpp#basic_char_array_writer, basic_char_array_writer>><__CharT__>`
| Writes to also `char*`, but does not append the termination character `'\0'`

| `<<destination_hpp#discarded_destination,discarded_destination>><__CharT__>`
| Discards content. The equivalent of writing to '/dev/null'.
...
template <typename CharT> class discarded_destination;
