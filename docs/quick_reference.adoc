...
:invalid_seq_notifier_c:  <<strf_hpp#invalid_seq_notifier_c,invalid_seq_notifier_c>>
:invalid_seq_notifier:    <<strf_hpp#invalid_seq_notifier,invalid_seq_notifier>>
:invalid_seq_notifier_ptr:  <<strf_hpp#invalid_seq_notifier_ptr,invalid_seq_notifier_ptr>>
...
The `{invalid_seq_notifier_c}` facet category enables you to
to specify a function to be called every time an invalid
sequence is detected. 

The example below throws an exception if `u16str` contains any invalid sequence:
[source,cpp,subs=normal]
----
#include <strf/to_string.hpp>

struct my_notifier: strf::{invalid_seq_notifier} {
...
int main() {
    ...
    auto str = strf::to_string.width(notifier_ptr)(strf::conv(u16str));
