...
| Same as in `trancode(cs)`, except that the transcoding has
  undefined behaviour if the input string is not fully conformant
  to the encoding represent by `cs`.
  It is expected to have better performace than `trancode`.
...
| `<<to_cfile_hpp#narrow_cfile_writer,narrow_cfile_writer>><__CharT__, __BufferSize__>`
...
* `__BufferSize__` is a `std::size_t` constexpr value;
...
An "invalid sequence" is any input that is non-conformant to the source character encoding,
or that cannot be to encoded into the destination encoding.
...
as https://timsong-cpp.github.io/cppwp/n4868/format#string.std-11[specified] to `std::format`,
