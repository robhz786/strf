...
|`<<to_print_dest_ref,to>> (__print_dest_ref__)`
| `<<strf_hpp#,<strf.hpp> >>`
...
* `__cfile__` is a `FILE*`
* `__print_dest_ref__` is a `{print_dest}<__CharT__>&`,  where `__CharT__` is a character type.
...
[subs=normal]
----
strf::to(__print_dest_ref__) (__args\...__)
----
[horizontal]
Return type:: `void`
Supports reserve:: No

See the <<destination_types,list of types>> that derive from `{print_dest}<__CharT__>&`.
====
...
The table below lists the concrete types that derivate from the `{print_dest}<__CharT__>` abstract class.
