...
** `PTraits` is `{printing_traits_of}<T>`
** `Fmts\...` is template paramenter pack used to define
   `PTraits::<<PrintingTraits_formatters,formatters>>` (
