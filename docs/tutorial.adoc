...
concrete class that derives from the `{basic_outbuff}` abstract
class template. These classes are also
<<quick_reference#outbuff_types, documented>>.
This means that you can write a function or code
that writes to a reference of `{basic_outbuff}`, so that
it works with all destination types:

[source,cpp,subs=normal]
----
void get_message(strf::basic_outbuff<char>& dest)
{
    strf::to(dest) ("Hello World!");
}
----
Hence, in a sense, `strf::{basic_outbuff}` is equivalent
to `std::basic_ostream` or `std::basic_streambuf`,
but with better performance, and it's easier to implement
a class that derives from `strf::{basic_outbuff}`
