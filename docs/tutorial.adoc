...
concrete class that derives from the `{print_dest}` abstract
...
that writes to a reference of `{print_dest}`, so that
it works with all kind of destinations:

[source,cpp,subs=normal]
----
void get_message(strf::print_dest<char>& dest)
{
    strf::to(dest) ("Hello World!");
}
----

Hence, in a sense, `strf::{print_dest}` is equivalent
to `std::basic_ostream` or `std::basic_streambuf`,
but with better performance, and it's easier to implement
a class that derives from `strf::print_dest`
