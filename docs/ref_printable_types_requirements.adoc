...
`PreMeasurements::<<premeasurements,size_required>>` is `true`:
...
template <precalc_size SizeRequired, precalc_width WidthRequired>
class premeasurements
    : public {size_accumulator}<static_cast<bool>(SizeRequired)>
    , public {width_decumulator}<static_cast<bool>(WidthRequired)>
{
public:
...
constexpr explicit premeasurements(<<width_t,width_t>> initial_width) noexcept;
----
Compile-time requirement:: `WidthRequired` is `precalc_width::yes`, otherwise this constructor
...
----
template < typename CharT
         , <<premeasurements,precalc_size>> SizeRequired
         , <<premeasurements,precalc_width>> WidthRequired
         , typename\... FPE
         , typename\... Args >
void measure
    ( {premeasurements}<SizeRequired, WidthRequired>* pp
...
Effects::
* When `SizeRequired` is `precalc_size::yes`, for each argument `arg` in `args\...`
...
----
(\..., {printer_type}< CharT
                  , {premeasurements}<SizeRequired, WidthRequired>
                  , facets_pack<FPE\...>
                  , Args >  {{make_printer_input}<CharT>(pp, facets, args)} );
----
* When `SizeRequired` is `precalc_size::no` and `WidthRequired` is `precalc_width::yes`,
...
* When `SizeRequired` is `precalc_size::no` and `WidthRequired` is `precalc_width::no`,
  does nothing
