...
public:
    ...
    explicit constexpr size_accumulator(std::ptrdiff_t initial_size) noexcept;
    ...
    constexpr void add_size(std::ptrdiff_t) noexcept;
...
does not participate in overload resolution.
Precondition:: `initital_size >= 0`
Postcondition:: `accumulated_ssize() == initial_size`
====
[[size_accumulator_add_size]]
====
[source,cpp]
----
constexpr void add_size(std::ptrdiff_t s) noexcept;
----
Precondition:: `s >= 0`

Effects::
When `Active` is `false`::: None
When `Active` is `true` ::: The internally stored size value ( that is returned by `accumulated_ssize()` ) is incremented by `s`.
====
...
constexpr std::size_t accumulated_usize() const noexcept;
----
Return value:: `static_cast<std::size_t>(accumulated_ssize())`
====
...
[source,cpp,subs=normal]
----
constexpr clear_negative_width() noexcept;
----
Effect:: Sets the internal `width_` to zero if it is negative;

Calling `subtract_width(w)` has undefined behavior if `w` is greater than
`(width_ - <<width_t,width_min>>)`.
So calling `clear_negative_width` aims to reduce the chances of that to happen.
====
