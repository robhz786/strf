...
So the first to learn is how {basic_outbuff} works.
...
`pointer()`. After that, you shall call the `<<outbuff_hpp#basic_outbuff_advance,advance>>`
...
memcpy(dest.pointer(), "hello", 5);
dest.advance(5);
----

As done above, before writting anything to `pointer()`, one
...
be _{OutbuffCreator}_, and it has the following effect when
writing the arguments ( when its member function `operator()`
or `tr` is called ):
+
[source,cpp,subs=normal]
----
typename your_outbuff_creator::outbuff_type dest{creator.create()};
// \... write content in dest \...
return dest.finish();
----
, where:
+
** `your_outbuff_creator` is the template argument ( and the
type defined in step 2). It must be be _{OutbuffCreator}_.
** `creator` is an object of type `your_outbuff_creator`.

* `{destination_calc_size}`: Its template argument must
be _{SizedOutbuffCreator}_, and it has the following effect when
writing the arguments:
+
[source,cpp,subs=normal]
----
std::size_t size = /{asterisk} calculate size \... {asterisk}/;
typename you_outbuff_creator::sized_outbuff_type dest{creator.create(**size**)};
// \... write content in dest \...
return dest.finish();
----

* `{destination_with_given_size}`: the factory must be _{SizedOutbuffCreator}_,
