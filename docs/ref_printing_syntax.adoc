...
image::syntax.svg[width=615,align=center]
...
struct reserve_given_space {
    explicit reserve_given_space(std::size_t s) : space(s) {}
...
[source,cpp,subs=normal]
----
constexpr /{asterisk} \... {asterisk}/  reserve(std::size_t space) /{asterisk} \... {asterisk}/;
----
...
[source,cpp,subs=normal]
----
constexpr       printing_syntax &  reserve(std::size_t space) &;
constexpr       printing_syntax && reserve(std::size_t space) &&;
----
...
[source,cpp,subs=normal]
----
constexpr printing_syntax reserve(std::size_t space) const &;
constexpr printing_syntax reserve(std::size_t space) const &&;
----
...
reserve(std::size_t space) const &;

constexpr printing_syntax<DestCreator, strf::reserve_calc, FPE\...>
reserve(std::size_t space) &&;
----
Returns a new `printing_syntax` whose internal `DestCreator` and `FPE\...` objects
are copy-constructed or move-constructed ( depedending on the overload used )
from the internal `DestCreator` and `FPE\...` objects of this object,
