...
struct reserve_given_space {
    constexpr explicit reserve_given_space(std::ptrdiff_t s) : space(s) {}
...
public:
    ...
    constexpr /{asterisk} \... {asterisk}/ <<printing_syntax_reserve,reserve>>(std::ptrdiff_t capacity) /{asterisk} \... {asterisk}/;
...
[source,cpp,subs=normal]
----
constexpr /{asterisk} \... {asterisk}/  reserve(std::ptrdiff_t space) /{asterisk} \... {asterisk}/;
----
...
[source,cpp,subs=normal]
----
constexpr       printing_syntax &  reserve(std::ptrdiff_t space) &;
constexpr       printing_syntax && reserve(std::ptrdiff_t space) &&;
----
...
[source,cpp,subs=normal]
----
constexpr printing_syntax reserve(std::ptrdiff_t space) const &;
constexpr printing_syntax reserve(std::ptrdiff_t space) const &&;
----
...
reserve(std::ptrdiff_t space) const &;

constexpr printing_syntax<DestCreator, strf::reserve_calc, FPE\...>
reserve(std::ptrdiff_t space) &&;
----
Returns a new `printing_syntax` whose internal `DestCreator` and `FPE\...` objects
are copy-constructed or move-constructed ( depedending on the overload used )
from the internal `DestCreator` and `FPE\...` objects of this object,
...
- `char_type`, a character type
- `size`, a value of the `std::ptrdiff_t`
