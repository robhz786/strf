...
using stringifier_t = {stringifier_type}<char_type, pre_t, FPack, parg_t>;
stringifier_t p{ {make_stringifier_input}<char_type>(pre, fpack, parg) };
...
stringifier_t p{ {make_stringifier_input}<char_type>(pre, fpack, arg) };
----
