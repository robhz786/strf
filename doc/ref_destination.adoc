...
Effect::
. Creates the <<basic_outbuff,outbuff>> object doing
+
[source,cpp,subs=normal]
----
...
using printer_type = {printer_impl}<char_type, preview_type, FPack, Arg>;
...
////
. Creates/get the <<basic_outbuff,outbuff>> object doing
+
[source,cpp,subs=normal]
----
typename OutbuffCreator::outbuff_type ob{_outbuff_creator.create()};
...
. Creates/get the <<basic_outbuff,outbuff>> object by doing
+
[source,cpp,subs=normal]
----
...
- `X` has the `X::size_outbuff_type` that is a type alias to a concrete type that is derives
   from `basic_outbuff<X::char_type>`
...
::
[horizontal]
Return type:: A type that derives from `{basic_outbuff}<char_type>`
