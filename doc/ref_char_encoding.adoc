...
using write_replacement_char_f = void ({asterisk}) ( {underlying_outbuff}<CharSize>& );
...
[horizontal]
Postcondition:: `ob.<<underlying_outbuff_recycle,recycle>>()` is not called if
`ob.<<underlying_outbuff_size,size>>()` is not less then the value returned by
...
- `X` is a __{UnderlyingCharEncoding}__ type for `sizeof(CharT)`
