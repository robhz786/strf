...
[horizontal]
`operator>({width_t} width)`::: Align to the right ( Or to the left right-to-left (RTL) script )
`operator<({width_t} width)`::: Align to the left ( Or to the right right-to-left (RTL) script )
`operator^({width_t} width)`::: Center alignment
`operator%({width_t} width)`::: Split the content, as in `std::internal`.
`fill(char32_t ch)`::: Set the fill character.
`set_alignment_format(.../{asterisk}see below{asterisk}/)`::: Specify alignment, width and fill character.
...
public:
    ...
    constexpr T&& <<float_formatter_fn_pad0,pad0>>(unsigned w) && noexcept;
