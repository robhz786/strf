...
[source,cpp,subs=normal]
----
auto wfunc = [](char32_t ch) \-> strf::<<reference#width_t,width_t>> {
    using namespace strf::<<reference#width_literal,width_literal>>;

    static const strf::<<reference#width_t,width_t>> roman_numerals_width [] = {
