...
----
When the last value is `-1`, the previous group is not repeated:
[source,cpp]
----
...
types, though you need to use the function `cv`:
...
The `sani` function has the same effect as `cv`,
except when the input encoding is same as the output.
In this case `sani` causes the input to be sanitized, whereas `cv` does not:
...
// convert to UTF-8
auto str_strict = strf::to_u8string(strf::cv(original));
auto str_lax =
    strf::to_u8string .with(strf::surrogate_policy::lax) (strf::cv(original));
...
auto utf16_lax =
    strf::to_u16string .with(strf::surrogate_policy::lax) (strf::cv(str_lax));
...
auto str = strf::to_string.width(notifier)(strf::cv(u16str));
----
...
`template <typename Encoding> cv(Encoding enc)`:::
