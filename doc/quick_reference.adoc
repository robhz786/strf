...
member functions below, following the
...
[source,cpp]
----
auto str = strf::to_string( +*strf::center(1.0, 15, U'~').sci() );
assert(str == "~~~~+1.e+01~~~~")
----
== Destinations
...
|`<<dest_outbuf,to>> (__outbuf_ref__) (__args\...__)`
| `<<strf_hpp#,<strf.hpp> >>`
...
* `__cfile__` is a `FILE*`
* `__outbuf_ref__` is a `basic_outbuf<__CharT__>&`,  where `__CharT__` is a character type.
...
[subs=normal]
----
strf::to(__outbuf_ref__) (__args\...__)
----
[horizontal]
Return type:: `basic_outbuf<__CharT__>&`
Return value:: `__outbuf_ref__`
Supports reserve:: No
See the <<outbuf_types,list of types>> that derive from `basic_outbuf<__CharT__>&`.
====
...
//-
Return type:: `<<outbuf_hpp#basic_cstr_writer,basic_cstr_writer<__CharT__>::result`>>
...
[[outbuf_types]]
== Derivates of `basic_outbuf`

The table below lists the concrete types that derivate from the `basic_outbuf<__CharT__>` abstract class.

[%header,cols="47,53"]
|===
| Type | Description

| `<<outbuf_hpp#basic_cstr_writer, basic_cstr_writer>><__CharT__>`
| Writes C strings

| `<<outbuf_hpp#discarded_outbuf,discarded_outbuf>><__CharT__>`
| Discard content
...
* `Traits` is a https://en.cppreference.com/w/cpp/named_req/CharTraits[_CharTraits_] type.
* `A` is an https://en.cppreference.com/w/cpp/named_req/Allocator[_Allocator_] type
...
template <typename CharT> class discarded_outbuf;
...
- It calls the `handle` function on the facet object correspoding to the `<<strf_hpp#tr_error_handling_c,tr_error_handling_c>>` category, which, by default, does nothing.
...
|`<<tr_string_error,tr_error_handling_c>>`   |No  |  Callback to notify errors on the tr-string
|===
