...
`<<outbuf_hpp#main,<strf/outbuf.hpp>>>` :: This is lighweight header can be used in freestanding evironments and is the cornerstone of library. All other headers include it.
`<<strf_hpp#main,<strf.hpp>>>` :: Defines most of the library, including the main usage syntax , all printable types and all facets.
////
It is influenced by the macros `STRF_SEPARATE_COMPILATION`, `STRF_FREESTANDING`, and `STRF_WITH_CSTRING`.
////
`<<to_string_hpp#main,<strf/to_string.hpp>>>` :: Provides utilities to write to `std::basic_string`. Includes `<<strf_hpp#main,<strf.hpp>>>`.
`<<to_streambuf_hpp#main,<strf/to_streambuf.hpp>>>` :: Provides utilities to write to `std::basic_streambuf`. Includes `<<strf_hpp#main,<strf.hpp>>>`.
`<<to_cfile_hpp#main,<strf/to_cfile.hpp>>>` :: Provides utilities to write to `FILE*`. Includes `<<strf_hpp#main,<strf.hpp>>>`.
...
void sample()
{
    ...
    std::string str =
        ...
        .with(strf::numpunct<10>{3}) // numeric punctuation
...
==== Header file `<strf/to_string.hpp>`
[source,cpp,subs=normal]
----
template < typename CharT
         , typename Traits = std::char_traits<CharT>
         , typename Allocator = std::allocator<CharT> >
constexpr <__/{asterisk} ... {asterisk}/__>  to_basic_string {};
...
----
::
Equivalent to `to(dest, dest_end - dest)`.
...
----
::
Equivalent to `to(dest, N)`.

==== Header file `<strf/to_cfile.hpp>`
...
==== Header file `<strf/to_streambuf.hpp>`
...
For example, `<<static_char_encodings,iso_8859_1>><char>` and
`<<static_char_encodings,utf8>><char>` are both facets of the category
`char_encoding_c<char>`.
...
|`<<invalid_seq_notifier,invalid_seq_notifier_c>>`    |Yes |Encoding error handling
|`<<surrogate_policy,surrogate_policy_c>>` |Yes |Wheter surrogates are treated as errors
|`<<tr_string_error,tr_error_handling_c>>`   |No  |Tr-string parsing error handling
|===

[[constrained_facets]]
==== Constrained facets

With the `constrain` function template you can create __constrained facets__, which are facets that only apply to certain printable types. Its template parameter is a template type that contains a `value` member constexpr convertible to bool that tells whether a given input type is under the influence of the given facet:
...
// punct_dec_2 overrides punct_dec_1 for all printable types,
...
The tr-string syntax is designed in such a way that there is only one possible kind of syntatic error, which is when the argument associated with a `"{"` does not exists. And when this happens, the library does two things:
...
- It calls the `handle` function on the facet object correspoding to the `<<strf_hpp#tr_error_handling_c,tr_error_handling_c>>` category, which, by default, does nothing. But this is a customization point. You can implement your own facet that instead throws an exception, or log the occurrence, or does something else, as you wish.
...
[source,cpp]
----
auto str = strf::to_string.with(numpunct<10>(1, 2, 3))(1000000000000ll);
assert(str == "1,000,000,000,00,0");
----
The value `-1` implies no further separator:
[source,cpp]
----
auto str = strf::to_string.with(numpunct<10>(1, 2, 3, -1))(1000000000000ll);
assert(str == "1000000,000,00,0");
----
...
<<printable_types, "printable types">>. But there is a set of format functions
that is common is most printable types, which are related to alignment
...
`conv()`::: Translates input string from the encoding associated to `CharT` to
