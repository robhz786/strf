...
std::string sample()
{
    ...
    std::string str =
        strf::to_string // the dispatcher
        .reserve(100)   // The reserved capacity in the created string
        .facets(strf::monotonic_grouping<10>{3}) // numeric punctuation
...
| `write(CharT* str, std::size)`
| `struct /{asterisk}{asterisk}/ { CharT* ptr; bool truncated; }`

| `write(CharT* begin, CharT* end)`
| `struct /{asterisk}{asterisk}/ { CharT* ptr; bool truncated; }`
...
| `write(std::basic_streambuf<CharT, Traits, Alloc>&)`
...
The "dispatcher" is what defines the effect and also the return type of the main usage syntax, as listed below:
...
[source,cpp,subs=normal]
----
/{asterisk}\...{asterisk}/ write ( char*     dest, std::size_t count );
/{asterisk}\...{asterisk}/ write ( char8_t*  dest, std::size_t count );
/{asterisk}\...{asterisk}/ write ( char16_t* dest, std::size_t count );
/{asterisk}\...{asterisk}/ write ( char32_t* dest, std::size_t count );
/{asterisk}\...{asterisk}/ write ( wchar_t*  dest, std::size_t count );
----
::
[horizontal]
...
[source,cpp,subs=normal]
----
/{asterisk}\...{asterisk}/ write ( char*     dest, char*     dest_end );
/{asterisk}\...{asterisk}/ write ( char8_t*  dest, char8_t*  dest_end );
/{asterisk}\...{asterisk}/ write ( char16_t* dest, char16_t* dest_end );
/{asterisk}\...{asterisk}/ write ( char32_t* dest, char32_t* dest_end );
/{asterisk}\...{asterisk}/ write ( wchar_t*  dest, wchar_t*  dest_end );
----
::
[horizontal]
Requirements::: `dest_end > dest`
Effect::: Same as in `write(dest, dest_end - dest)`
Return type::: Same as in `write(dest, dest_end - dest)`
Return value::: Same as in `write(dest, dest_end - dest)`
Support reserve::: No


[source,cpp,subs=normal]
----
template<std::size_t N> /{asterisk}\...{asterisk}/ write ( char     (&dest)[N] );
template<std::size_t N> /{asterisk}\...{asterisk}/ write ( char8_t  (&dest)[N] );
template<std::size_t N> /{asterisk}\...{asterisk}/ write ( char16_t (&dest)[N] );
template<std::size_t N> /{asterisk}\...{asterisk}/ write ( char32_t (&dest)[N] );
template<std::size_t N> /{asterisk}\...{asterisk}/ write ( wchar_t  (&dest)[N] );
----
::
[horizontal]
Effect::: Same as in `write(dest, N)`
Return type::: Same as in `write(dest, N)`
Return value::: Same as in `write(dest, N)`
Support reserve::: No

[source,cpp,subs=normal]
----
template <typename CharT = char>
/{asterisk}\...{asterisk}/ write(std::FILE* dest);
----
::
[horizontal]
...
[source,cpp,subs=normal]
----
/{asterisk}\...{asterisk}/ wwrite(std::FILE* dest);
----
::
[horizontal]
...
/*...*/ write(std::basic_streambuf<CharT, Traits>& dest);
----
::
[horizontal]
...
[[reserve]]
=== Reserving
The `reserve` and `reserve_calc` are only supported in some dispatcher types, as indicated above.
...
__facet objects__ to the <<usage_syntax,`facets`>> function.
...
=== Single characters
Type:: `CharT`, that is the output character type defined by the <<dispatchers,dispatcher>>.
...
where:
::
- `CharT` is the output character type ( defined by the <<dispatchers,dispatcher>> )
...
=== Strings encoded differently

The `cv` function template enables you to pass a string based on a different encoding and character type as an input type. See <<encoding_conversion, Enconding conversion>>.

The `fmt_cv` function enables you to apply <<alignment_format_function, alignment format functions>>.

[source,cpp]
----
auto str = strf::to_string(strf::fmt_cv(u" Hello! ").fill('*') ^ 16);
assert(str == "**** Hello! ****");
----
...
inner_pack</*...*/> facets(const Facets& ... facets);
----
.Example 1
[source,cpp]
----
