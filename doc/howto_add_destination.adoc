...
= Extending Strf &#x2014; How to add a new destination
...
Creating a new output type envolves defining a concrete class
...
becames greater than or equal to `{min_size_after_recycle}()`.
...
private:
    ...
    char buff[strf::{min_size_after_recycle}()];
};
----
...
intended to create a `QString` object instead of a `std::string`.
...
( because it needs to allocate memory or whatever reason ).
...
Where the class created in step 2 is the template paramenter.
...
return dest.finish();
----
where `creator` is a private member object of the type defined in step 2.
...
The reason why it was created was to define the return type of
...
struct foo_writer_creator {
    using outbuff_type = foo_writer;
    using char_type = char;
    foo& create() const { return f; }
    foo& f;
}
