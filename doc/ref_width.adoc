...
:width_and_len: <<width_and_len,width_and_len>>
...
[source,cpp]
----
x.str_width_and_len(charset, limit, str, str_len, surr_poli)
----
[horizontal]
...
Return type::: `{width_and_len}`
Return value::: A value `ret` of type `{width_and_len}`, such that
* `ret.len` is the greatest value that is less than or equal to `str_len`
  such that `x.width(charset, width_max, str, ret.len, surr_poli).width`
  is less than or equal to `limit`.
* `ret.width` is the width of the substring of `str` with size `ret.len`
...
////
==== Struct `width_and_len` [[width_and_len]]
...
public:
    ...
    template <typename Charset>
    constexpr {width_and_len} <<fast_width_str_width_and_len,str_width_and_len>>
        ( const Charset&
        , {width_t} limit
        , const {underlying_char_type}<Charset::char_size>*
        , std::size_t str_len
...
* `limit`, otherwise
====
[[fast_width_str_width_and_len]]
====
[source,cpp,subs=normal]
----
template <typename Charset>
constexpr {width_and_len} str_width_and_len
    ( const Charset&
    , {width_t} limit
    , const {underlying_char_type}<Charset::char_size>*
    , std::size_t str_len
...
* if `limit \<= 0` is `true`, then `ret.width` and `ret.len` are `0`;
* otherwise, if `limit.floor()` is less than `str_len`,
`ret.width` and `ret.len` are equal to `limit.floor()`;
* otherwise, `ret.width` and `ret.len` are equal to `str_len`.
====
...
public:
    ...
    template <typename Charset>
    constexpr {width_and_len} <<width_as_fast_u32len_str_width_and_len,str_width_and_len>>
        ( const Charset&
        , {width_t} limit
        , const {underlying_char_type}<Charset::char_size>*
        , std::size_t str_len
...
* `charset.<<UnderlyingCharset_codepoints_fast_count,codepoints_fast_count>>(str, str + str_len, limit.floor()).count`, otherwise.
====
[[width_as_fast_u32len_str_width_and_len]]
====
[source,cpp,subs=normal]
----
template <typename Charset>
...
* If `limit \<= 0` is `true`, then then `ret.width` and `ret.len` are `0`;
* otherwise, `ret.width` is equal to `ret2.count` and `ret.len` is equal
  to `ret2.pos - str`, where `ret2` is the return value of
  `charset.<<UnderlyingCharset_codepoints_fast_count,codepoints_fast_count>>(str, str + str_len, lim)`
====
...
public:
    ...
    template <typename Charset>
    constexpr {width_and_len} <<width_as_u32len_str_width_and_len,str_width_and_len>>
        ( const Charset&
        , {width_t} limit
        , const {underlying_char_type}<Charset::char_size>*
        , std::size_t str_len
...
Return value:::
* `0`, if `limit <= 0` is `true`
* `charset.<<UnderlyingCharset_codepoints_robust_count,codepoints_robust_count>>(str, str + str_len, limit.floor()).count`, otherwise.
====
[[width_as_u32len_str_width_and_len]]
====
[source,cpp,subs=normal]
----
template <typename Charset>
...
* otherwise, `ret.width` is equal to `ret2.count` and `ret.len` is equal
  to `ret2.pos - str`, where `ret2` is the return value of
  `charset.<<UnderlyingCharset_codepoints_robust_count,codepoints_robust_count>>(str, str + str_len, lim)`
====
...
public:
    ...
    template <typename Charset>
    constexpr {width_and_len} <<width_by_func_str_width_and_len,str_width_and_len>>
        ( const Charset&
        , {width_t} limit
        , const {underlying_char_type}<Charset::char_size>*
        , std::size_t str_len
...
charset.<<UnderlyingCharset_to_u32,to_u32>>().<<UnderlyingTranscoder_transcode,transcode>>
    ( ob, str, str + str_len, {invalid_seq_policy}::replace, surr_poli )
----
...
====
[[width_by_func_str_width_and_len]]
====
[source,cpp,subs=normal]
----
template <typename Charset>
...
----
charset.<<UnderlyingCharset_to_u32,to_u32>>().<<UnderlyingTranscoder_transcode,transcode>>
    ( ob, str, str + ret.len, {invalid_seq_policy}::replace, surr_poli )
----
, where `ob` is any object whose type derives from `{underlying_outbuf}<4>`.
* `ret.len` is the greatest value that is less than or equal to `str_len`
  such that `str_width_and_len(charset, limit, str, ret.len, surr_poli).width`
  is less than or equal to `limit`.
====
