...
:fmt_type:        <<strf_hpp#PrintTraits_fmt_type,fmt_type>>
...
////
To create format functions you need to define a class complying with
the __{Formatter}__ requirements. But you may also or
instead consider using some of those already available
by the library.
There are two of them that seem to make sense for `std::complex`:
`{alignment_formatter}` and `{float_formatter}`.

Once having all formatters, you need to define the
`fmt_type` member type alias in your _PrintTraits_ class
and overload `make_printer_input`
////

Format functions are defined by classes that
comply with the __{Formatter}__ requirements.
If you want to add format functions
you need to create your a formatter class and/or
use formatters provided by the library.
There are two of them that make sense for `std::complex`:
`{alignment_formatter}` and `{float_formatter}`.

Now, to specify which __formatters__ ( and thus which format functions )
are aplicable to your printable type, we need to define the `{fmt_type}`
member type alias in its __PrinterTraits__ class.
It must be an instance of `value_with_format` with
the first template paramenter being its __PrinterTraits__ class,
while the others are the __{Formatter}__ types:
...
struct print_traits<std::complex<FloatT>> {
    ...
    using {fmt_type} = strf::{value_with_formatters}
        < print_traits<std::complex<FloatT>>
        , {alignment_formatter}
...
are all well-formed, whenever `x` is a `std::complex` value.
...
Note that we can not just use instead `fmt_type` here. We need to add that
template parameter pack because the __Formatters__ types in `{value_with_formatters}`
...
the format functions are defined. The template parameter needs to
be used to in the return type of the format functions:
...
modifications: `fmt_type` and the first line
