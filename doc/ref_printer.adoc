...
template <typename CharT, typename Preview, typename FPack, typename Arg>
constexpr auto make_printer_input(Preview& preview, const FPack& facets, const Arg& arg)
{
    auto tag = typename {print_traits_of}<Arg>::<<PrintTraits,facet_tag>>;
    auto f = {get_facet}<{print_override_c}, tag>(facets);
    return f.template make_printer_input<CharT>(preview, facets, arg);
}
