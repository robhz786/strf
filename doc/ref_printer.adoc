...
*  otherwise, it is `Traits`, when `T` is `{value_with_format}<Traits, /{asterisk}\... {asterisk}/>`
...
* `forwarded_printable_type<T>` is `T`, when `T` a `{value_with_format}</{asterisk}\... {asterisk}/>`
...
T::fmt_type;
----
A type alias to a `{value_with_format}<T, /{asterisk}\...{asterisk}/>` type.
