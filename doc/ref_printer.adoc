...
T::override_tag
----
When defined, `{make_printer_input}` uses the following expression
to obtain the _PrinterInput_ object:
[source,cpp,subs=normal]
----
<<get_facet,get_facet>><{print_override_c}, override_tag}(_facets_).template make_printer_input(...)
----
...
[source,cpp,subs=normal]
----
{print_traits_of}<Arg>::template <<PrintTraits_functions,make_printer_input>><CharT>(preview, facets, arg);
----
...
struct no_print_override {
    ...
    template <typename CharT, typename Preview, typename FPack, typename Arg>
    constexpr static auto make_printer_input(Preview& preview, const FPack& facets, Arg&& arg)
...
[source,cpp,subs=normal]
----
{print_traits_of}<Arg>::template make_printer_input<CharT>(preview, facets, arg)
----
