...
struct codepoints_count_result {
    std::size_t count;
    const {underlying_char_type}<CharSize>{asterisk} pos;
};

template <std::size_t CharSize>
using codepoints_fast_count_f =
    codepoints_count_result<CharSize> ({asterisk})
...
[source,cpp]
----
x.codepoints_fast_count(begin, end, max_count)
----
::
[horizontal]
Return type:: `{codepoints_count_result}<CharSize>`
Return value:: `{c, pos}`, where:

* `c` is the number of Unicode code points
  in the range `[begin, end)`, if such value is less than `max_count`.
  Otherwise `c` is equal to `max_count`.
* `pos` is the position such that the number of codepoints in
  the range `[begin, pos)` is equal to `c`.

Posconditions::

* `0 \<= c && c \<= max_count` is `true`
* `begin \<= pos && pos \<= end` is `true`
...
[source,cpp]
----
x.codepoints_robust_count(begin, end, max_count, surr_poli)
----
::
[horizontal]
...
* `c` is equal to `std::min(max_count, u32len)`, where `u32len` is the
   ...
   `[begin, end)` from this charset to UTF-32, following the
   <<surrogate_policy, policy>> of `surr_poli`
   and of `{invalid_seq_policy}::replace`.
* `pos` is the position such that the value of
  `x.codepoints_robust_count(begin, pos, (std::size_t)-1, surr_poli).count`
   is equal to `c`.
Posconditions::
* `0 \<= c && c \<= max_count` is `true`
* `begin \<= pos && pos \<= end` is `true`
...
----
[horizontal]
Effect:: Calls and returns `this\->data\->codepoints_fast_count_func(begin, end, max_count)`.
====
====
[source,cpp,subs=normal]
----
...
[horizontal]
Effect:: Calls and returns
        `this\->data\->codepoints_robust_count_func(begin, end, max_count, surr_poli)`.
====
====
[source,cpp,subs=normal]
----
