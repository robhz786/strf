...
int main()
{
    {
        char buff[200];
        using t1 = decltype(strf::format(buff) .exception("aaaa", 555));
        using t2 = decltype(strf::format(buff) ("{}...{}").exception("aaaa", 555));
        using t3 = decltype(strf::make_string.exception("aaaa", 555));
        using t4 = decltype(strf::make_string("{}{}").exception("aaaa", 555));
    ...
    {
        char buff[200];
        strf::format(buff) .exception("aaaa", 555);
    ...
    {
        auto str = strf::make_string.exception("aaaa", 555);
        BOOST_TEST(str == "aaaa555");
    }

    {
        auto str = strf::make_string ("{} {}").exception("aaa", "bbb");
...

    {
        ...
        try
        {
            strf::format(buff) .exception("aaa", erroneous_arg);
        }
...

    {
        ...
        try
        {
            auto str = strf::make_string.exception("aaa", erroneous_arg);
        }
        catch (const std::system_error& sys_err)
        {
            err_code = sys_err.code();
